...
const restoredBytes = await vlfs.read(vlfs.open('/save/save.ksd',0),2);
vlfs.registerOverlayFile('/savedata/restored.ksd',restoredBytes);
const restored = vlfs.open('/savedata/restored.ksd',0);
assert.deepEqual(Array.from(vlfs.readCached(restored,2)),[1,2]);
vlfs.close(restored);
let saved;
vlfs.onWriteClose = (path,data) => {saved = {path,data};};
const rewritten = vlfs.open('/savedata/restored.ksd',1);
vlfs.write(rewritten,restoredBytes); vlfs.close(rewritten);
assert.equal(saved.path,'/savedata/restored.ksd');
assert.deepEqual(Array.from(saved.data),[1,2]);
